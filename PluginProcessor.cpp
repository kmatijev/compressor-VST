...
void BasicCompressorAudioProcessor::parameterChanged(const juce::String& parameterId, float newValue)
{
    updateParameters();
}

void BasicCompressorAudioProcessor::updateParameters()
{
    // koristimo nativne set metode
    // iz value tree-a uzimamo parametre za svaki parametar te ih onda postavljamo na parametre kompresora 
    // i na parametre ulaznog i izlaznog gain module-a
...
{
    // prepare to play funkcija se koristi kako bismo proslijedili sample rate i velicinu audio blokova
    // ti podatci su nuzni jer se koristi takozvani "smoothing" -> kako bi audio signal bio "smooth" potrebno
    // je znati sample rate audio block i njegovu velicinu
    ...
    inputModule.setRampDurationSeconds(0.02);   // ramp odreduje koliko vremena treba parametru da se pomakne od izvorne
                                                // vrijednosti do one novo postavljene
    outputModule.prepare(spec);
    outputModule.setRampDurationSeconds(0.02);

    compressorModule.prepare(spec);

    updateParameters(); // inicijalizacija parametera prije nego sto se plugin ucita
}
